...
#' 'id' must also be a variable in the metadata table.
...
#' @param different_sources_flag A boolean value indicating the source-sink assignment.
...
#' If \eqn{Pij} == NA it indicates that source j was not used in the analysis of sink i.
#'
#' @examples
#' \donttest{
#'
...
FEAST <- function(C
  ...
  if(sum(as.integer(C) != as.numeric(C)) > 0){
    stop('Data must be integer. Consider using "ceiling(datatable)" or ceiling(1000*datatable) to convert floating-point data to integers.')
  }
  ...
  if(length(common.sample.ids) <= 1) {
    message <- paste(sprintf('Error: there are %d sample ids in common between the metadata file and data table', common.sample.ids))
    stop(message)   
  }
